...
export const RangerStats = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const getUserInfo = async (userId) => {
                if (!userId) return { name: "Nieznany", email: "Brak e-maila", clubName: "—" };
                ...
                try {
                    ...
                    if (userDocSnap.exists()) {
                        ...
                        const clubName = userData.association_club_name || "—";
                        const info = { name, email: email.split("@")[0], clubName };
                ...
                const fallback = { name: "Nieznany", email: "Brak e-maila", clubName: "—" };
    ...
    const downloadCSV = () => {
        ...
        const csvData = filteredStats.map(ranger => ({
            ...
            "Koło Strażnika": ranger.clubName || "—",
        ...
        const sorted = [...filteredStats].sort((a, b) => {
            if (sortConfig.key === 'email') {
                ...
                return sortConfig.direction === 'asc'
                    ? aValue - bValue
                    : bValue - aValue;
            } else if (sortConfig.key === 'name' || sortConfig.key === 'clubName') {
                // String sorting for name and club name
                    ...
                    <thead>
                        <tr>
                            <th onClick={() => handleSort('name')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                                Strażnik {sortConfig.key === 'name' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : '⇅'}
                            </th>
                            <th onClick={() => handleSort('email')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                                ID Strażnika {sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : '⇅'}
                            </th>
                            <th onClick={() => handleSort('clubName')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                                Koło Strażnika {sortConfig.key === 'clubName' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : '⇅'}
                            </th>
